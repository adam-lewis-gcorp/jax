...
def make_sharded_device_array(
    ...
    # The Optional exists only to be backward compatible with an old signature
    # (aval, device_buffers).
  ...
  This is to be used only within JAX. It will return either a C++ SDA or a
  Python DeviceArray when the buffers are not JAX buffers.
...
if _xla_extension_version >= 30:
  ShardedDeviceArrayBase = pmap_lib.ShardedDeviceArrayBase
  ...
  ShardedDeviceArrayBase.__bases__ = ((xla.DeviceArray,) +
                                      ShardedDeviceArrayBase.__bases__)
  base_class = ShardedDeviceArrayBase
else:
  base_class = xla.DeviceArray
...
if _xla_extension_version >= 30 and _USE_EXPERIMENTAL_CPP_SDA:
  ShardedDeviceArray = ShardedDeviceArrayBase
else:
