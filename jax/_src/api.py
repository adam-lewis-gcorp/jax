...
def pmap(
  ...
  **Multi-process platforms:** On multi-process platforms such as TPU pods,
  ...
  [ 13.  13.]
  """
  if FLAGS.experimental_cpp_pmap and config.omnistaging_enabled:
    func = _cpp_pmap
  else:
    func = _python_pmap
...
def _cpp_pmap(
  ...
  def cache_miss(_, *args, **kwargs):
    ...
    # We can always use the C++ fast path. execute will be None when we are
    # only tracing.
    ...
    if use_fastpath:
      ...
      fastpath_data = {
          "version": 1,  # For forward and backward compatibility
  ...
  static_broadcasted_argnums_ = (0,) + tuple(
      i + 1 for i in static_broadcasted_tuple)
  cpp_mapped_f = pmap_lib.pmap(fun, cache_miss, static_broadcasted_argnums_)
  ...
  def f_pmapped(*args, **kwargs):
    ...
    return cpp_mapped_f(context, *args, **kwargs)
